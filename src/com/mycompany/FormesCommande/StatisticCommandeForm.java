...
public class StatisticCommandeForm extends BaseFormC {
  
        
    private boolean drawOnMutableImage;
   
    
    private double nbrclientcommande ;
    private double nbrtotalcommande ;
    int nbp;int nbg;
   
    Form current;
    BaseFormC form;
        public StatisticCommandeForm(Resources res,int nbpp,int nbgg)  {
        ...
        addTab(swipe, res.getImage("AyoubBack.jpg"), spacer1, "Bienvenue");
        ...
        RadioButton all = RadioButton.createToggle("Statistique du client", barGroup);      
       ...
       /* RadioButton popular = RadioButton.createToggle("Categorie Reclamation", barGroup);
        ...
        RadioButton profile = RadioButton.createToggle("Tous Les commandes", barGroup);
       ...
       /* bindButtonSelection(popular, arrow);
                all.addActionListener((e)->{new StatisticCommandeForm(res,nbp,nbg).show();
        });*/
                bindButtonSelection(profile, arrow);
                profile.addActionListener((e1)->{new ListAllCommandeForm(res).show();
        });
  ...
  ;       
    ...
    public void createPieChartForm() {
        ...
        nbrclientcommande=nbp;
        nbrtotalcommande=nbg-nbp;
        double total =  nbg;
        
        //values
        double prcntFeed = (nbrclientcommande *100)/total;
        
        double prcntRec = (nbrtotalcommande * 100)/total;
        ...
        String []messages = {
            "Statistique NB Commandes du client choisit % Reste des client"
        };
    ...
    private CategorySeries buildDataset(String title, double prcntFeed, double prcntRec) {
        ...
        series.add("% Le Reste",prcntRec);
        series.add("% Ses Commandes",prcntFeed);
